...
```
packages/css/
├── globals.css          # Shared Tailwind entry + base styles + token mapping
├── light.css            # Light theme tokens
└── dark.css             # Dark theme tokens

apps/desktop/src/
└── quick-pane.css       # Quick pane-specific window overrides
```

**Multi-window theming**: all windows should consume `packages/css/globals.css`, which imports both `light.css` and `dark.css`. When adding new color variables, update the shared files under `packages/css` rather than re-defining tokens in app-local CSS.
...
Place the light token in `packages/css/light.css` and the dark token in `packages/css/dark.css`.
