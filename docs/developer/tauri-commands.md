...
If the command returns user-visible strings, localize them through `rust-i18n` rather than hardcoding English:
...
#[tauri::command]
#[specta::specta]
pub async fn my_new_command() -> Result<(), String> {
    Err(t!("errors.example.failed"))
}
```
